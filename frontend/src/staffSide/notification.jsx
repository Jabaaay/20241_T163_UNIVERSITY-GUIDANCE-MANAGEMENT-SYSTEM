...
function Status({ updateUnreadCount }) {
  ...
  const deleteNotification = async (id) => {
    try {
      const response = await fetch(`http://localhost:3001/staff/contact/${id}`, {
        method: 'DELETE',
      });
      if (response.ok) {
  ...
  const handleNotificationClick = async (notification) => {
    try {
      const response = await fetch(`http://localhost:3001/staff/contact/${notification._id}`, {
        method: 'PATCH',
  ...
  const handleMarkAllAsRead = async () => {
    try {
      const response = await fetch('http://localhost:3001/staff/contact/markAllAsRead', {
        method: 'PATCH',
      });
      if (response.ok) {
        setNotifications((prev) =>
